...
export class CalculatorComponent implements OnInit {
  ...
  private _filter(value: string): string[] {
    ...
    return Object.keys(this.kommuner)
      .filter((kommun) => kommun.toLowerCase().startsWith(filterValue))
      .sort();
  }

  onTypeKommun() {
    this.selectedK = Object.keys(this.kommuner).includes(
      this.myForm.get('kommun')?.value
    );
    if (!this.selectedK)
      this.myForm.controls['kommun'].setErrors({ incorrect: true });
  ...
  private updateForsamlingar() {
    if (this.selectedK && this.selectedY) {
      this.forsamlingar = this.kommuner[this.myForm.get('kommun')?.value]
        .filter((row: any) => row['år'] == this.myForm.get('year')?.value)
        .map((row: any) => row['församling'])
        .sort();
  ...
  handleSubmit() {
    if (this.myForm.valid) {
      if (
        this.myForm.get('medlem')?.value &&
        !this.forsamlingar.includes(this.myForm.get('forsamling')?.value)
      ) {
  ...
  onCalculate() {
    ...
    if (this.selectedY && this.selectedK && this.myForm.get('medlem')?.value) {
      taxRate = this.kommuner[kommun]
        .filter((row: any) => row['år'] == year)
        .filter((row: any) => row['församling'] == forsamling)
        .map((row: any) => +row['summa, inkl. kyrkoavgift'])[0];
    } else {
      let array = this.kommuner[kommun].filter((row: any) => row['år'] == year);
      taxRate =
        +array[0]['kommunal-skatt'] +
        +array[0]['landstings-skatt'] +
        +array.reduce(
          (acc: number, row: any) => acc + +row['begravnings-avgift'],
          0
        ) /
          array.length;
    }
    ...
    this.resultArray = [
      +tax.toFixed(2),
      +nettoinkomst.toFixed(2),
      +inkomst.toFixed(2).toLocaleString('en-US'),
      +((tax / inkomst) * 100).toFixed(2),
    ];
  }
}
